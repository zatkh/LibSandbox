...
static struct elf_module *elf_module_alloc(const char *name)
{
    ...
    m = memdom_alloc(elf_mdom, sizeof(*m));
    if (m == NULL) {
...
static bool layout_segments(struct elf_module *m, struct elf_info *info)
{
    ...
    mm_start = memdom_mmap(elf_mdom,NULL,
                    m->size,
...
static bool elf_link(struct elf_module *m)
{
    ...
    /* load needed module */
    pneeded = needed = (struct elf_module **)memdom_alloc(elf_mdom,(1 + m->needed_count) * sizeof(struct elf_module *));
    if (needed == NULL) {
